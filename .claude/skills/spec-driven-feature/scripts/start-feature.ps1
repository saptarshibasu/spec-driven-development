...
# number, slugify the description, create specs/<NNN-slug>/, and copy the three
...
foreach ($f in 'spec.template.md', 'plan.template.md', 'tasks.template.md') {
...
# 4. Copy the three templates and fill in only the mechanical header fields
#    (feature id, title, date) — never the actual content.
$today = Get-Date -Format 'yyyy-MM-dd'
foreach ($doc in 'spec', 'plan', 'tasks') {
