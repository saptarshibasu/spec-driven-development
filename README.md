...
This also seeds `.agents/model-map.conf` — the org-specific model/tool policy the mirror scripts
need to run — with the kit's defaults, but only on first adoption: if the file is already there,
it's left alone, so your edits survive every later update.
...
| `.agents/model-map.conf` | **Seeded, not overwritten** — copied in only if missing, then yours to edit; later updates leave it alone. |
